...
async function openTab(title, url) {
    ...
    try {
        if (url.endsWith(".pdf")) {
            ...
            iframe.src = url; iframe.style.width="100%"; iframe.style.height="100%"; iframe.style.flex="1";
            iframe.frameBorder="0"; contentElem.appendChild(iframe);
        }
    } catch(err){ contentElem.innerHTML=`<p style="color:red;">Failed to load: ${err.message}</p>`; }
...
async function openResourceTab(title, resource){
    ...
    try{
        ...
        videoLink.href="#"; videoLink.textContent="Click here to play video"; videoLink.style.display="block"; videoLink.style.marginBottom="10px"; videoLink.style.fontWeight="bold";
...
document.addEventListener("DOMContentLoaded",()=>{
    document.querySelectorAll(".nav-link").forEach(link=>{
        link.addEventListener("click",function(e){
            ...
            else { openTab(title,this.getAttribute("data-url")); }
        });
    });
    ...
    function openSearchTab(){
        ...
        const inputBox=document.createElement("input"); inputBox.type="text"; inputBox.id="search-box"; inputBox.placeholder="Search...";
        ...
        contentElem.appendChild(inputBox); contentElem.appendChild(resultsDiv);
        ...
        inputBox.addEventListener("keypress",function(e){
            if(e.key==='Enter'){
                ...
                results.forEach(res=>{
                    ...
                    item.addEventListener("click",()=>{ if(doc.resource){ openResourceTab(doc.title,doc.resource);} else if(doc.url){ openTab(doc.title,doc.url); }});
    ...
    // 在 Tab Bar 添加 Search 按钮
    const searchBtnTab=document.createElement("div");
    searchBtnTab.className="tab"; searchBtnTab.style.marginLeft="auto"; searchBtnTab.textContent="🔍 Search";
